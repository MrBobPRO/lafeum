...
<section class="search">
    <div class="select2_single_container">
        <select class="select2_single select2_single_linked" data-placeholder="Поиск..."
            ...
            <option value="{{ route('dashboard.quotes.single', $item->id)}}">{{$item->body}}</option>
            @endforeach
        </select>
    </div>
</section>
...
<section class="list">
    ...
    <div class="titles">
        <div class="titles__item width-25">
            @if($order_by != "author_name")
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=author_name&order_type=asc' }}">Автор
                <span class="material-icons-outlined titles__icon">arrow_upward</span>
            </a>
            @elseif($order_by == "author_name" && $order_type == "asc")
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=author_name&order_type=desc' }}">Автор
                <span class="material-icons-outlined titles__icon titles__icon--active">arrow_upward</span>
            </a>
            @elseif($order_by == "author_name" && $order_type == "desc")
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=author_name&order_type=asc' }}">Автор
        ...
        <div class="titles__item width-25">Цитата</div>

        <div class="titles__item width-25">
            @if($order_by != "category_names")
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=category_names&order_type=asc' }}">Категории
                <span class="material-icons-outlined titles__icon">arrow_downward</span>
            </a>
            @elseif($order_by == "category_names" && $order_type == "asc")
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=category_names&order_type=desc' }}">Категории
                <span class="material-icons-outlined titles__icon titles__icon--active">arrow_upward</span>
            </a>
            @elseif($order_by == "category_names" && $order_type == "desc")
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=category_names&order_type=asc' }}">Категории
                <span class="material-icons-outlined titles__icon titles__icon--active">arrow_downward</span>
            </a>
        ...
        <div class="titles__item width-25">
            ...
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=created_at&order_type=asc' }}">Дата добавления
                <span class="material-icons-outlined titles__icon">arrow_upward</span>
            </a>
            @elseif($order_by == "created_at" && $order_type == "asc")
            <a class="titles__link"
                href="{{ route('dashboard.index') . '?page=' . $active_page . '&order_by=created_at&order_type=desc' }}">Дата добавления
                <span class="material-icons-outlined titles__icon titles__icon--active">arrow_upward</span>
            </a>
    ...
    <form action="{{ route('quotes.remove_multiple') }}" method="POST" id="multiple_items_form">
        @csrf
        ...
        <div class="list__item">
            ...
            <div class="checkbox">
                <label for="{{$quote->id}}" class="checkbox__label">
                    <input class="checkbox__input" id="{{$quote->id}}" type="checkbox" name="ids[]"
                        value="{{$quote->id}}">
                    <span class="checkbox__checkmark"></span>
                </label>
            </div>

            <div class="list__item-div width-25">{{$order_by == "author_name" ? $quote->author_name : $quote->author->name}}</div>
            <div class="list__item-div width-25">{{$quote->body}}</div>
            <div class="list__item-div width-25">
                @if($order_by == 'category_names')
                    {{-- Add space between categories --}}
                    {{ str_replace(',', ', ', $quote->category_names)}}
                @else
            ...
            <div class="list__item-controls">
                <a class="control-button control-button--blue" href="{{ route('quotes.single', $quote->id)}}"
                ...
                <a class="control-button" href="{{route('dashboard.quotes.single', $quote->id)}}" data-bs-toggle="tooltip"
                ...
                <button class="control-button control-button--red" type="button" data-action="show_single_remove_modal"
                    data-item-id="{{$quote->id}}" data-bs-toggle="tooltip" data-bs-placement="bottom"
    ...
    {{$quotes->links("templates.pagination")}}
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Вы уверены что хотите удалить отмеченные цитаты ?<br><br>
            </div>
...
<div class="modal fade" id="remove_single_modal" tabindex="-1" aria-labelledby="remove_single_modal_label"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="button" data-bs-dismiss="modal">Отмена</button>
                <form action="{{ route('quotes.remove') }}" method="POST">
                    {{ csrf_field() }}
